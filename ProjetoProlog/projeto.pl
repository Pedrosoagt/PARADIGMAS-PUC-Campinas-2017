...
%pergunta roteiro(Origem, Destino, ListaVoos).
	...
	roteiro(O,D):- voodireto(O,D,_,_,_),!.
	roteiro(O,D):- voodireto(O,Z,_,_,_),roteiro(Z,D).
...
%pergunta menorDuracao(Origem,Destino,dia,HoraSaida,HoraChegada,Companhia).
	...
	menorduracaoaux(O,D,Cod,H:M,(_,HoraC:MinC),_,Comp,Dia,Duração):-
		voo(O,D,Cod,Hora:Min,(DiaC,HoraC:MinC),_,Comp,Dias),
		membro(Dias,Dia),soma60(Hora,Min,DuraçãoS),
